...
class TfidfGuesser:
    ...
    def train(self, training_data) -> None:
        ...
        for i in range(len(x_array)):
            sents = nltk.sent_tokenize(x_array[i])
            for s in sents:
                q_text = s.split()
                label = i
                data.append((q_text, label))

        voc, word2ind, ind2word = dan.load_words(data)

        dan_model = dan.DanModel(len(answer_docs.items()), len(voc));

        dan_model.to(device)
        # pdb.set_trace()

        train_dataset = dan.Question_Dataset(data, word2ind)
        ...
        for epoch in range(5):
            ...
            train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=256,
                                               ...
                                               collate_fn=dan.batchify)
            accuracy = dan.train(None, dan_model, train_loader, dev_loader, accuracy, device)
