...
def train(args, model, train_data_loader, dev_data_loader, accuracy, device):
    ...
    for idx, batch in enumerate(train_data_loader):
        ...
        if idx % 50 == 0 and idx > 0:
            print_loss_avg = print_loss_total / 50
...
class DanModel(nn.Module):
    ...
    def __init__(self, n_classes, vocab_size, emb_dim=50,
        ...
        self.classifier = nn.Sequential(
                nn.Linear(emb_dim, n_hidden_units),
                nn.ReLU(),
                nn.Linear(n_hidden_units, 100),
                nn.ReLU(),
                # nn.Linear(n_hidden_units, 1000))
